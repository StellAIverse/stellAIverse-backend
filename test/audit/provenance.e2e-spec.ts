...
describe("ProvenanceController (e2e)", () => {
  ...
  describe("GET /provenance", () => {
    ...
    it("should return 401 without authentication", () => {
      return request(app.getHttpServer()).get("/provenance").expect(401);
    });
    ...
    it("should filter by agentId", async () => {
      ...
      expect(
        response.body.data.every((r: any) => r.agentId === "agent-test"),
      ).toBe(true);
    });
    ...
    it("should filter by status", async () => {
      ...
      expect(response.body.data.every((r: any) => r.status === "success")).toBe(
        true,
      );
    });
  ...
  describe("GET /provenance/agents/:agentId", () => {
    ...
    it("should return provenance for a specific agent", async () => {
      ...
      expect(
        response.body.data.every((r: any) => r.agentId === "agent-specific-2"),
      ).toBe(true);
    });
  });
  ...
  describe("ProvenanceService", () => {
    it("should update a provenance record", async () => {
      ...
      const updated = await provenanceService.updateProvenanceRecord(
        record.id,
        {
          status: ProvenanceStatus.SUCCESS,
