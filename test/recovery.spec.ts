...
describe("RecoveryService", () => {
  ...
  const mockChallengeService = {
    issueChallengeForAddress: jest
      .fn()
      .mockReturnValue("Sign this message to authenticate: abc123"),
  };
  ...
  describe("requestRecovery", () => {
    it("should send recovery email for verified account", async () => {
      jest
        .spyOn(emailLinkingService, "getUserByEmail")
    ...
    it("should throw error for non-existent email", async () => {
      ...
      await expect(
        service.requestRecovery("nonexistent@example.com"),
    ...
    it("should normalize email to lowercase", async () => {
      ...
      expect(emailLinkingService.getUserByEmail).toHaveBeenCalledWith(
        "test@example.com",
      );
    });
  });
  ...
  describe("verifyRecoveryAndGetChallenge", () => {
    it("should return challenge for verified email", async () => {
      ...
      const result =
        await service.verifyRecoveryAndGetChallenge("test@example.com");
