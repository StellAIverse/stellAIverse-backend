...
describe("NonceGuard", () => {
  ...
  beforeEach(async () => {
    const module: TestingModule = await Test.createTestingModule({
      providers: [
        NonceGuard,
        {
          provide: getRepositoryToken(SubmissionNonce),
          useValue: mockNonceRepository,
        },
      ],
    }).compile();
  ...
  it("accepts first-time nonce", async () => {
    ...
    const result = await guard.canActivate(
      makeContext({ userId: "u1", nonce: 1 }),
    );
    expect(result).toBe(true);
  });

  it("accepts higher nonce", async () => {
    mockNonceRepository.findOne.mockResolvedValue({
      address: "u1",
      nonce: "5",
    });
    mockNonceRepository.save.mockResolvedValue({});
