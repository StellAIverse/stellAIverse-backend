...
export class AaveAdapter implements ProtocolAdapter {
  ...
  async getPosition(
    ...
    try {
      ...
      this.logger.error(
        `Error getting position for ${address} on ${token}`,
        error,
      );
      throw error;
    }
  ...
  async deposit(
    ...
    try {
      ...
      const tx = await lendingPool.deposit.populateTransaction(
        tokenAddress,
        amountWei,
        address,
        0,
  ...
  async withdraw(
    ...
    try {
      ...
      const tx = await lendingPool.withdraw.populateTransaction(
        tokenAddress,
        amountWei,
        address,
      );
  ...
  async borrow(
    ...
    try {
      ...
      const tx = await lendingPool.borrow.populateTransaction(
        tokenAddress,
        amountWei,
        interestRateMode,
        0,
  ...
  async repay(
    ...
    try {
      ...
      const tx = await lendingPool.repay.populateTransaction(
        tokenAddress,
        amountWei,
        interestRateMode,
        address,
  ...
  async getCollateralData(
    ...
    try {
      ...
      return {
        ...
        availableToBorrow: Math.max(
          0,
          Number(ethers.formatEther(totalCollateral)) -
  ...
  async getRewards(
    ...
    try {
      ...
      const rewardsBalance = await rewardsController.getRewardsBalance(
        addresses,
        user,
      );
  ...
  async claimRewards(
    ...
    try {
      ...
      const aTokens = positions.map((p) =>
        this.getATokenAddress(p.token, chain),
      );

      const tx = await rewardsController.claimRewards.populateTransaction(
        aTokens,
        ethers.MaxUint256,
        address,
      );
  ...
  async getRiskMetrics(
    ...
    return {
      ...
      liquidationRisk: Math.min(
        100,
        (collateral.ltv / collateral.maxLtv) * 100,
      ),
      counterpartyRisk: 20,
  ...
  private async getATokenAddress(
    token: string,
    chain: string,
  ): Promise<string> {
